class Solution {
    ...
    public int closedIsland(int[][] grid) {
        ...
        for(int i = 0 ; i < rowL ; i++){
            if(grid[i][0]==0)dfs(i,0,visited,grid);
            if(grid[i][colL-1] == 0) dfs(i , colL-1 , visited , grid);
            
        }for(int j=0;j<colL;j++){
            if(grid[0][j]==0)dfs(0,j,visited,grid);
            if(grid[rowL-1][j]==0)dfs(rowL-1,j,visited,grid);
        }int count=0;
        for(int i=0;i<rowL;i++){
            for(int j=0;j<colL;j++){
